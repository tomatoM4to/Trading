...
## 5. 트러블슈팅 및 마이너 픽스 기록 (Troubleshooting)
배포 자동화 과정에서 발생할 수 있는 엣지 케이스를 방어하기 위해 다음과 같은 설정이 반영되어 있습니다.
- **GitHub Actions (비대화형 쉘) 방어**: `init-cert.sh` 스크립트를 통해 초기 SSL 발급 시, CI/CD 러너에서는 TTY 입력이 불가능하므로 `docker run` 명령어에서 `-it`(대화형) 옵션을 제거하여 `input device is not a TTY` 에러를 방지합니다.
- **역할 분리 (Separation of Concerns)**: `init-cert.sh`는 순수하게 인증서만 발급하고 종료하며, 실제 서비스(`docker compose up -d`) 구동은 메인 배포 파이프라인(`deploy.yml`)으로 위임하여 워크플로우의 책임을 분리했습니다.
- **Sudo 환경 변수 유지**: OCI 서버에서 `docker login ghcr.io` 실행 시 비밀번호 등 환경 변수가 파이프(`|`)로 넘어갈 때 유실되는 것을 막기 위해 `sudo -E` 옵션을 명시적으로 사용하여 권한 상승 시에도 환경 변수를 보존합니다.
- **도커 이미지 태그 단순화**: `IMAGE_NAME`을 리포지토리명과 동일하게 일치시켜 도커 허브(GHCR)의 패키지 네이밍 규칙과 충돌하지 않도록 구성했습니다.
