...
- 수렴 필터의 `threshold`가 handler에서 숫자 검증과 SQL 바인딩 없이 쿼리 문자열에 들어간다.
- 이격도 handler는 검증 후 `ma_daily_5`가 `ma5`로 바뀐 값에서 일봉 여부를 판별하여 일봉 요청을 분봉으로 취급할 수 있다.
- 별도 자동 회귀 테스트가 없어 AST 경계와 AND/OR 의미론을 지속적으로 보장하지 못한다.

이 문서는 현재 상태를 설명하므로 결함을 정상 계약으로 간주하지 않는다. 관련 코드를 수정할 때 위 항목을 테스트와 함께 제거한다.
