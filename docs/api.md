...
현재 관리자 라우터에는 별도 인증 dependency가 없고 Nginx는 모든 경로를 프록시한다. 따라서 운영 배포에서는 `/admin/test`, `/admin/action`, `/docs`, `/redoc`이 외부에 노출될 수 있다. 인증 또는 네트워크 제한이 구현되기 전까지 이 경로를 공개 관리 API로 간주해서는 안 된다.
