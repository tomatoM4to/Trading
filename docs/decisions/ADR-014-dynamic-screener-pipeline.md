...
3. **SQLite Push-down**: 기술적 지표(이평선 정배열, 크로스 등) 연산은 파이썬으로 데이터를 끌어오지 않고, SQLite 내부의 Window Function(`LAG`, `ROW_NUMBER`, `AVG OVER`)과 CTE를 활용하여 단일 쿼리로 필터링을 완수(Push-down)한다. 분봉 추세와 일봉 추세를 모두 단일 패턴으로 지원한다.
