...
- **백그라운드 OHLCV 데이터 적재 스케줄러 (Daily & Minute)**:
  ...
  - **무료 API 한계 극복 (Retry 전략)**: KIS API의 간헐적 접속 끊김(`Read timed out`) 발생 시 1초 대기 후 최대 3회 재시도.
  - **고속 덮어쓰기 (UPSERT)**: 중복 데이터 수집을 방어하고 수정주가 등 최신 정보를 업데이트하기 위해 `INSERT OR REPLACE` 기반 벌크 인서트 적용.
- **관리자 라우터 및 무결성 검증 (Admin & Test)**:
  - `app/routes/admin.py`를 통해 스케줄러 수동 제어 제공.
  - `/check-daily-ohlcv`, `/check-minute-ohlcv`: 적재된 최신 캔들의 날짜(Date) 분포율을 계산하여 업데이트 누락을 자동 진단. (분봉 데이터는 수집 시간차에 의한 자연스러운 분산을 고려하여 판별).
  - `/verify-minute-integrity`: 실시간 KIS API 호출 결과와 내부 DB에 적재된 1분봉 데이터(가격/거래량)를 1:1로 비교 검증하여 데이터 무결성을 100% 증명.
