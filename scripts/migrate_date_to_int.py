...
def migrate():
    ...
    """)
    conn.execute(
        "INSERT INTO daily_ohlcv_new SELECT ticker, CAST(date AS INTEGER), open, high, low, close, volume, amount FROM daily_ohlcv"
    )
    ...
    """)
    conn.execute(
        "INSERT INTO minute_ohlcv_new SELECT ticker, CAST(date AS INTEGER), time, open, high, low, close, volume, amount FROM minute_ohlcv"
    )
