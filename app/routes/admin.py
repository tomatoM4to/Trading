...
async def verify_minute_integrity(ticker: str = None, market: str = "KOSPI"):
    """
    ...
    try:
        ...
        # 1. 종목 지정이 없으면 DB에 있는 해당 시장 종목 중 무작위 1개 추출
        if not ticker:
            cursor.execute("""
            ...
            row = cursor.fetchone()
            if not row:
                raise HTTPException(status_code=404, detail=f"No tickers found in DB for {market}.")
            ticker = row['ticker']
