...
# scripts 디렉토리 모듈 임포트
from scripts.kis_kosdaq_code_mst import get_kosdaq_master_dataframe, kosdaq_master_download
...
def init_stock_codes_db():
    ...
    # 1. KOSPI 필터링
    kpi = parsed_kospi_df
    kpi = kpi[kpi['그룹코드'].str.strip() == 'ST']  # 보통주(주식)만 남김 (펀드, ETF, ETN, 리츠 등 완벽 제거)
    ...
    kpi_cols = {
        '단축코드': 'ticker', '한글명': 'name', 'market': 'market',
        '전일거래량': 'prev_vol', '시가총액': 'market_cap', '상장주수': 'total_shares',
        '자본금': 'capital', '신용가능': 'credit_able', '증거금비율': 'margin_rate'
    }
    ...
    kdq_cols = {
        '단축코드': 'ticker', '한글종목명': 'name', 'market': 'market',
        '전일 거래량': 'prev_vol', '전일기준 시가총액 (억)': 'market_cap', '상장 주수(천)': 'total_shares',
        '자본금': 'capital', '신용주문 가능 여부': 'credit_able', '증거금 비율': 'margin_rate'
    }
    ...
    try:
        ...
        logger.info(f"Successfully initialized {len(combined_df)} stock codes into database from CSV.")
